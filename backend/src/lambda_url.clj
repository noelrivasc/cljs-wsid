...
(defn- lambda-url-request->ring-request [lambda-url-request]
  {:pre [(do
           (println "what we got" lambda-url-request)
           (println "the method" (get-in lambda-url-request [:requestContext :http :method]))
           (println "the raw path" (get-in lambda-url-request [:rawPath]))
           true)
         (every? #(get-in lambda-url-request %) [[:requestContext :http :method] [:rawPath]])
